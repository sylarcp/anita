...
@api.route('/<ip_db>/g12_pos/nbufs/<start_time>')
def get_g12_pos_nbufs(ip_db, start_time):
    try:
        g12_poss =getattr(G12_pos,ip_db).with_entities(G12_pos.nbuf, G12_pos.now, G12_pos.time).filter(G12_pos.time>start_time).order_by(G12_pos.now).limit(200).all()
        ...
        return jsonify({})
@api.route('/<ip_db>/g12_sat/nbufs/<start_time>')
def get_g12_sat_nbufs(ip_db, start_time):
    try:
        g12_sats =getattr(G12_sat,ip_db).with_entities(G12_sat.nbuf, G12_sat.now, G12_sat.time).filter(G12_sat.time>start_time).order_by(G12_sat.now).limit(200).all()
